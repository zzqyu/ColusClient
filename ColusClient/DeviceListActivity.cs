...
namespace ColusClient
{
    ...
    public class DeviceListActivity : Activity
    {
        ...
        public const string EXTRA_CONNECT_CODE = "connect_code";
        ...
        {
            int result;
            EditText code = FindViewById<EditText>(Resource.Id.input_connect_code);
            if (code.Text == "" /*|| !int.TryParse(code.Text, out result)*/)
            {
                Toast.MakeText(this, "연결코드를 입력하세요. ", ToastLength.Long);
            }
            else
            {
                ...
                intent.PutExtra(EXTRA_CONNECT_CODE, code.Text);
