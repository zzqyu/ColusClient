...
namespace ColusClient
{
    ...
    public partial class MainActivity : AppCompatActivity, BottomNavigationView.IOnNavigationItemSelectedListener
    {
        ...
        {
            string[] title = new string[] {"마우스", "키보드", "PPT모드", "멀티미디어", "PC기능" };
            int tabIndex = -1;
            ...
            else if (item.ItemId == Resource.Id.navigation_multi) tabIndex = (3);
            else if (item.ItemId == Resource.Id.navigation_pcfunc) tabIndex = (4);
            if (tabIndex != -1)
            {
